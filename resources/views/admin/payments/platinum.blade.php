...
<body>
    <form id="payment-form" action="{{ route('admin.payments.checkout') }}" method="post" enctype="multipart/form-data">
        @csrf
        <h2 class="brand-text-color-1 ">Stai acquistando il <span class="ms-platinum"> {{$sponsorships[2]->name}}</span></h2>
        <p class="brand-text-color-1 fw-bold">
            Con <span class="ms-platinum">il pacchetto Platinum </span>, il tuo profilo professionale sarà messo in risalto sulla homepage della nostra piattaforma per <span class="text-danger">144 ore</span>. Questa visibilità privilegiata aumenterà l'esposizione del tuo profilo agli utenti, migliorando significativamente la tua presenza online e aumentando le possibilità di attrarre nuovi pazienti.
        </p>
        <p class="brand-text-color-1 fw-bold">
        ...
        <input type="hidden" name="sponsorship_id" value="{{ $sponsorships[2]->id }}">
        <input type="hidden" name="sponsorship_name" value="{{ $sponsorships[2]->name }}">
        <input type="hidden" name="profile_id" value="{{ $user->id }}">
        <input type="hidden" name="amount" value="{{$sponsorships[2]->price}}">
